...
def get_majority_y(user_labels, source_num, category_num):
    if not scipy.sparse.issparse(user_labels):
        ...
        majority_y = np.divide(
            majority_y,
            np.matlib.repmat(np.sum(majority_y, 1, keepdims=True), 1, category_num),
        )
    else:
        ...
        majority_y = np.matmul(user_labels, template)
        majority_y = np.divide(
            majority_y, np.matlib.repmat(np.sum(majority_y, 1), 1, category_num)
        )
    return majority_y
...
def gen_data(filename, data_num, source_num, category_num):
    ...
    _tmp = np.random.multinomial(
        1, [1.0 / category_num] * category_num, size=data_num * source_num
    )
    for i in range(data_num):
        for j in range(source_num):
            data_label_vectors[i, category_num * j : category_num * (j + 1)] = _tmp[
                i * source_num + j, :
            ]
    data_y_labels = np.argmax(
        np.random.multinomial(1, [1.0 / category_num] * category_num, size=data_num),
        axis=1,
    )
    np.savez(
        filename,
        data=data_label_vectors,
        labels=np.reshape(data_y_labels, (data_num, 1)),
    )
